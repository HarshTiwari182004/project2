...
public class PlaylistDAOModel
{
    ...
    public List<SongModel> getSongsInPlaylist(int playlistID, SongDAOModel songDAO) {
        List<SongModel> songsInPlaylist = new ArrayList<>();
        try (Connection connection = getConnection()) {
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
                ...
                while (resultSet.next()) {
                    ...
                    SongModel song = songDAO.getSongByID(songID);  // Fetch song using SongDAO
                    if (song != null) {
